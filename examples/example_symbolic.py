...
def example_infinite_sets():
    ...
    x = symbols('x')
    R = C.create_symbolic_set("ℝ", condition=x >= -oo)
    
    # Create the interval [0, 1]
    unit_interval = C.create_symbolic_set("[0,1]", expr=Interval(0, 1))
    
    # Create a morphism f: ℝ → [0,1] using sigmoid
    sigmoid = Lambda(x, 1 / (1 + exp(-x)))
    f = C.create_symbolic_morphism(R, unit_interval, sigmoid, "σ")
...
def example_universal_property_verification():
    ...
    P = C.create_symbolic_set("A×B", expr=FiniteSet(*product_elements))
    ...
    pi1 = C.create_morphism(P, A, {(a,b): a for (a,b) in product_elements}, "π₁")
    pi2 = C.create_morphism(P, B, {(a,b): b for (a,b) in product_elements}, "π₂")
...
1. INFINITE OBJECTS: Represent ℝ, intervals, and other
